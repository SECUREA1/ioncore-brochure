...
from ioncore_branding import BrandingContext, apply_ioncore_branding, brand_subtitle
...
class BluetoothApp:
    ...
    def _refresh_branding_context(self):
        self.branding = apply_ioncore_branding(self.root, mode=self.theme_mode)
        logo = getattr(self.branding, "logo", None)
        if logo is not None:
            try:
    ...
    def _init_branding(self):
        ...
        fonts = self.branding.fonts
        subtitle_text = brand_subtitle(
            "Unified telemetry for Bluetooth, Wi‑Fi, and cellular intelligence."
        )
        ...
        if self.branding.logo is not None:
            self.logo_label = tk.Label(
                self.branding_frame,
                image=self.branding.logo,
                borderwidth=0,
                highlightthickness=0,
                bg=colors["surface_alt"],
            )
            self.logo_label.image = self.branding.logo
        else:
            self.logo_label = tk.Label(
                self.branding_frame,
                text="IONCORE",
    ...
    def apply_theme(self):
        ...
        if hasattr(self, "logo_label"):
            ...
            if self.branding.logo is not None:
                self.logo_label.configure(image=self.branding.logo)
                self.logo_label.image = self.branding.logo
        self.branding_title.configure(bg=colors["surface_alt"], fg=colors["text"], font=fonts["title"])
        self.branding_tagline.configure(bg=colors["surface_alt"], fg=colors["muted_text"], font=fonts["subtitle"])
        self.theme_status.configure(bg=colors["surface_alt"], fg=colors["accent_alt"], font=fonts["small"])
    ...
    def toggle_theme(self):
        ...
        self._refresh_branding_context()
        self.apply_theme()
    ...
    def __init__(self, root):
        ...
        self.branding: Optional[BrandingContext] = None
        ...
        self.device_list.grid(row=0, column=0, columnspan=16, padx=5, pady=5)
        ...
        self._register_theme_widget(self.scan_button, "buttons")
        ...
        self._register_theme_widget(self.scan_wifi_button, "buttons")
        ...
        self._register_theme_widget(self.scan_cell_button, "buttons")
        ...
        self._register_theme_widget(self.test_button, "buttons")
        ...
        self._register_theme_widget(self.connect_button, "buttons")
        ...
        self._register_theme_widget(self.disconnect_button, "buttons")
        ...
        self._register_theme_widget(self.explore_button, "buttons")
        ...
        self._register_theme_widget(self.obex_button, "buttons")
        ...
        self._register_theme_widget(self.pbap_button, "buttons")
        ...
        self._register_theme_widget(self.map3d_button, "buttons")
        ...
        self._register_theme_widget(self.open_map_button, "buttons")
        ...
        self._register_theme_widget(self.relabel_button, "buttons")
        ...
        self._register_theme_widget(self.clearlabel_button, "buttons")
        ...
        self._register_theme_widget(self.scan_paired_button, "buttons")

        # --- New: Download/Export controls across the app ---
        ...
        self._register_theme_widget(self.download_button, "buttons")

        self.export_all_button = tk.Button(self.device_frame, text="Export ALL Radios (ZIP)", command=self.download_all_radio_data)
        self.export_all_button.grid(row=2, column=4, padx=5, pady=5)
        self._register_theme_widget(self.export_all_button, "buttons")

        self.save_log_button = tk.Button(self.device_frame, text="Save Log", command=self.save_log_to_file)
        self.save_log_button.grid(row=2, column=5, padx=5, pady=5)
        self._register_theme_widget(self.save_log_button, "buttons")

        self.save_map_html_button = tk.Button(self.device_frame, text="Save Map HTML", command=self.save_interactive_map)
        self.save_map_html_button.grid(row=2, column=6, padx=5, pady=5)
        self._register_theme_widget(self.save_map_html_button, "buttons")
        ...
        self._register_theme_widget(self.send_button, "buttons")
        ...
        self._register_theme_widget(self.receive_button, "buttons")
        ...
        self._register_theme_widget(self.advertise_button, "buttons")
    ...
    def row_text_for_wifi(self, info, status):
        ...
        return (
            ...
            f"  {dist_str}, Q:{qual}) - Freq {freq} MHz ch {chan} - Security {sec}"
        )
    ...
    def start_explore_ble(self):
        ...
        # New: Save snapshot of current explorer text
        tk.Button(btn_frame, text="Save Snapshot", command=self.save_explorer_snapshot).pack(side="left", padx=6)
        self.run_coro(self.populate_ble_services())

    def save_explorer_snapshot(self):
        if not self.explorer_text:
            messagebox.showwarning("No Data", "Open the BLE Explorer and load services first.")
            return
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        dest = filedialog.asksaveasfilename(
            defaultextension=".txt",
            initialfile=f"ble_services_{ts}.txt",
            filetypes=[("Text", "*.txt")]
        )
        if not dest:
            return
        try:
            text = self.explorer_text.get("1.0", tk.END)
            with open(dest, "w", encoding="utf-8") as f:
                f.write(text)
            self.log(f"Saved BLE Explorer snapshot -> {dest}")
            messagebox.showinfo("Saved", f"Saved to:\n{dest}")
        except Exception as e:
            self.log(f"Save snapshot failed: {e}")
    ...
    def open_map3d_popup(self):
        ...
        # New: Save snapshot (PNG)
        tk.Button(ctrl, text="Save 3D Snapshot", command=self.save_map3d_snapshot).pack(side="left", padx=6)
    ...
    def save_map3d_snapshot(self):
        if not self.map3d["fig"]:
            messagebox.showwarning("No Figure", "Open the 3D Map first.")
            return
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        dest = filedialog.asksaveasfilename(defaultextension=".png",
                                            initialfile=f"map3d_snapshot_{ts}.png",
                                            filetypes=[("PNG Image", "*.png")])
        if not dest:
            return
        try:
            self.map3d["fig"].savefig(dest, dpi=150)
            self.log(f"Saved 3D map snapshot -> {dest}")
            messagebox.showinfo("Saved", f"Saved to:\n{dest}")
        except Exception as e:
    ...
    def _toggle_sat_overlay(self):
        if self.map3d["sat_overlay_var"].get():
            # Load in a background thread then refresh
    ...
    def save_interactive_map(self):
        """Generate the interactive map and save to a user-picked HTML file."""
        if not HAVE_FOLIUM:
    ...
    def _generate_interactive_map(self, out_path="radio_map.html"):
        ...
        for typ, dev in self.devices:
            if typ == "BLE":
                ...
                # Cell is location-unknown; draw a conservative ring
                add_circle(self.map3d["cell_default_dist"], "#c62828", "Cellular approx range")

        m.save(out_path)
        return out_path

    # ---------- Export / Download (UPDATED/EXPANDED) ----------
    def save_log_to_file(self):
        """Save the current log panel to a text file."""
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        if not dest:
            return
        try:
            # Reading from a disabled widget is allowed
    ...
    def download_info(self):
        ...
        Practical rules:
          • If a selection exists -> export that one item with structured fields (CSV/JSON).
          • If no selection -> export exactly what is displayed in the list (summary lines).
        """
        try:
            # Nothing to export?
    ...
    def _export_text(self, dest, idxs):
        ...
        if idxs:
            try:
                # Use the actual displayed row text for the selected index
    ...
    def _build_struct_for(self, typ, id_):
        """
        Build a structured dictionary for a single selected item, pulling from your
        existing metadata stores. Always includes a 'summary' string (as shown in the UI).
        """
        now_iso = datetime.datetime.now().isoformat(timespec="seconds")
        base = {"type": typ, "id": id_, "timestamp": now_iso}

        # Status defaults mirror your list rendering
    ...
    def _row_text_by_id(self, typ, id_):
        """
        Recreate the row text for an item by ID. Prefer existing helpers.
        If an exact device object isn't found (rare), synthesize a concise line.
        """
        try:
            if typ == "BLE":
                dev = None
    ...
    def download_all_radio_data(self):
        """
        Export ALL currently known radio data (BLE, Classic, Wi‑Fi, Cellular) into a single ZIP archive.
        Contents:
          • all_data.json  (structured, with GPS origin and timestamp)
          • ble.csv, classic.csv, wifi.csv, cell.csv
          • displayed_list.txt  (exactly what the UI shows)
          • log.txt  (current log panel)
          • ble_services_last.txt (if BLE Explorer is open)
        """
        if not self.devices:
        ...
        data = {
            ...
            "ble": [], "classic": [], "wifi": [], "cell": []
        }

        # Collect records by category
        for typ, dev in self.devices:
            try:
                if typ == "BLE":
                    addr = dev.address
                    ...
                    data["classic"].append(rec)
                elif typ == "WiFi":
                    bssid = dev["bssid"]; m = self.wifi_meta.get(bssid, {}) or {}
                    rec = {
                        "bssid": bssid,
                    ...
                    data["wifi"].append(rec)
                elif typ == "Cell":
                    cid = dev["cid"]; m = self.cell_meta.get(cid, {}) or {}
                    rec = {
                        "cid": cid,
                        "operator": m.get("operator"),
                        "tech": m.get("tech"),
                ...
                # Keep exporting even if one record misbehaves
                continue
        ...
        # Helper: write CSV text to zip from list of dicts
        ...
        try:
            with zipfile.ZipFile(dest, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                ...
                zf.writestr(
                    ...
                    _csv_bytes(
                        data["ble"],
                        prefer_cols=["address", "name", "status", "rssi_dbm", "tx_power_ref_dbm",
                                     "distance_m", "range_label", "quality_grade", "addr_type",
                                     "service_uuid_count", "service_uuids", "manufacturer_ids",
                                     "gps", "paired", "device_id"]
                    )
                )
                zf.writestr(
                ...
                zf.writestr(
                    ...
                    _csv_bytes(
                        data["wifi"],
                        prefer_cols=["bssid", "ssid", "status", "rssi_dbm", "distance_m", "range_label",
                                     "quality_grade", "freq_mhz", "channel", "security"]
                    )
                )
                zf.writestr(
                    "cell.csv",
                    _csv_bytes(
                        data["cell"],
                        prefer_cols=["cid", "operator", "tech", "status", "signal_pct",
                                     "rssi_dbm", "rsrp_dbm", "rsrq_db", "sinr_db", "quality_bucket"]
                    )
                )

                # Displayed list snapshot
                ...
                # If BLE explorer is open, include its text
                try:
            ...
            self.log(f"Export ALL failed: {e}")
    ...
    def on_close(self):
        ...
        # Stop WinRT geolocation listeners (ADDED)
        try:

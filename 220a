...
class BluetoothApp:
    def __init__(self, root):
        ...
        self.branding_header = ioncore_branding.build_branding_header(
            ...
            subtitle="Real-time Bluetooth, Wi‑Fi, and cellular control with Ioncore Index polish.",
        )
        ...
        self.device_list.grid(row=0, column=0, columnspan=13, padx=5, pady=5)

        self.scan_button = tk.Button(self.device_frame, text="Scan Devices (BLE/Classic)", command=self.start_scan_devices)
        ...
        self.relabel_button.grid(row=1, column=10, padx=5, pady=5)

        self.clearlabel_button = tk.Button(self.device_frame, text="Clear Label", command=self.clear_label_selected_device)
        self.clearlabel_button.grid(row=1, column=11, padx=5, pady=5)

        # Optional explicit button (kept, but also run automatically):
        self.scan_paired_button = tk.Button(self.device_frame, text="Scan Paired (Windows)", command=self.start_scan_paired_windows)
        self.scan_paired_button.grid(row=1, column=12, padx=5, pady=5)
        ...
        self.advertise_button = tk.Button(self.device_frame, text="Advertise as 'Bullish'", command=self.start_advertising)
        ...
        self.map3d = {
            ...
            "wifi_default_dist": 10.0,    # default if Wi-Fi lacks RSSI
            "cell_default_dist": 120.0,   # conservative visualization for cell
    ...
    async def scan_devices(self):
        ...
        # keep wifi/cell meta between BLE/Classic scans; they’re refreshed by their own scans
    ...
    async def test_connection(self):
        ...
        if device_type == "BLE":
            ...
            # Wi-Fi and Cell aren’t connect-tested here
            result = "N/A"
    ...
    def row_text_for_cell(self, info, status):
        ...
        # We generally avoid “distance” for cellular; show bucket + metrics instead
        extra = []
        if pct is not None:
            extra.append(f"{pct}%")
        if rssi is not None:
            extra.append(f"RSSI {rssi} dBm")
        if rsrp is not None:
            extra.append(f"RSRP {rsrp} dBm")
        if rsrq is not None:
            extra.append(f"RSRQ {rsrq} dB")
        if sinr is not None:
            extra.append(f"SINR {sinr} dB")
    ...
    def clear_label_selected_device(self):
        ...
        for i, (t, d) in enumerate(self.devices):
            ...
            if t == "Classic" and d["addr"] == addr:
                ...
                self.update_device_list_entry(i, self.connection_results.get(addr, "Not Tested"), d, "Classic")
            if t == "WiFi" and d.get("bssid") == addr:
                # nothing to fallback; leave SSID as discovered
                self.update_device_list_entry(i, self.connection_results.get(addr, "Seen"), d, "WiFi")
            if t == "Cell" and d.get("cid") == addr:
                self.update_device_list_entry(i, self.connection_results.get(addr, "Seen"), d, "Cell")
    ...
    def open_map3d_popup(self):
        ...
        ax.set_title("Radios as trees (direction from ID, depth from distance/estimate)")
    ...
    def _collect_positions(self):
        ...
        # Cellular (visualize conservatively)
        for typ, dev in self.devices:
            if typ != "Cell":
                continue
            cid = dev["cid"]
    ...
    def _render_map3d(self):
        ...
        if xs_ble:
            ax.scatter(xs_ble, ys_ble, zs_ble, s=30, marker="o", color="#66bb6a", label="BLE")
        if xs_wifi:
            ax.scatter(xs_wifi, ys_wifi, zs_wifi, s=30, marker="s", color="#ba68c8", label="Wi-Fi")
        if xs_cell:
            ax.scatter(xs_cell, ys_cell, zs_cell, s=30, marker="x", color="#ef5350", label="Cellular")
        if xs_cla:
            ax.scatter(xs_cla, ys_cla, zs_cla, s=30, marker="^", color="#64b5f6", label="Classic")
    ...
    def _merge_wifi_in_main(self, nets):
        """Merge Wi-Fi scan results into devices & metadata, then refresh the UI."""
        added = 0
        for n in nets:
            bssid = n["bssid"]
            # store/refresh meta
            ...
            # append device record if not listed
    ...
    def scan_wifi_sync(self):
        ...
        try:
            ...
            nets = []
        # Post-process with smoothing + distance model
        out = []
        for n in nets:
            bssid = n["bssid"]
            rssi = n.get("rssi")
            ...
            # Wi-Fi distance clamp a bit wider than BLE
        ...
        for line in out.splitlines():
            ...
            if low.startswith("ssid "):
                ...
                if nets:
                    ...
                    # netsh reports Wi-Fi %; rough approx to dBm:
                    dbm = (pct / 2.0) - 100.0
    ...
    def _scan_wifi_macos(self):
        ...
        for ln in lines:
            ...
            # Security is generally at the end; airport can compact it
            # We’ll surface what’s available
    ...
    def scan_cell_sync(self):
        ...
        try:
            ...
            cells = []
        # Normalize quality buckets if missing
        for c in cells:
    ...
    def _cell_quality_bucket(self, c):
        """Bucketize based on best available metric (RSRP > RSSI > %)."""
        rsrp = c.get("rsrp_dbm")
        rssi = c.get("rssi_dbm")
        pct = c.get("signal_pct")
        # LTE/NR RSRP reference:
        # Excellent ≥ -80, Good -80..-90, Fair -90..-100, Poor -100..-110, Very poor < -110
        if rsrp is not None:
            v = rsrp
    ...
    def _scan_cell_windows(self):
        if not shutil.which("netsh"):
            return []
        # Enumerate interfaces to build stable CIDs
        ...
        for iface in interfaces or [{"name": "Cellular"}]:
            ...
            # Parse netsh output: it commonly exposes "Signal strength : NN%"
            pct = None
            operator = None
            tech = None
            ...
            cells.append({
                ...
                "tech": tech,
                "signal_pct": pct,
                "rssi_dbm": None,   # not exposed by netsh mbn
                "rsrp_dbm": None,
                "rsrq_db": None,
                "sinr_db": None,
                "qual_bucket": None
    ...
    # ----- macOS (no standard CLI for cellular) -----
    def _scan_cell_macos(self):
        # Most Macs have no WWAN; external modems often lack a unified CLI.
        # Try to detect via 'scutil --nc list' for known WWAN services (no signal metrics).
    ...
    def _scan_cell_linux(self):
        ...
        cells = []
        for path in paths:
            # Basic modem info (operator, access tech)
            ...
            for ln in sig.splitlines():
                ...
                if "rssi" in s and "dbm" in s and rssi is None:
                    try:
                        rssi = float(re.search(r"(-?\d+\.?\d*)\s*dBm", ln, re.I).group(1))
                    except Exception:
                        pass
                if "rsrp" in s and rsrp is None:
                    try:
                        rsrp = float(re.search(r"(-?\d+\.?\d*)\s*dBm", ln, re.I).group(1))
                    except Exception:
                        pass
                if "rsrq" in s and rsrq is None:
                    try:
                        rsrq = float(re.search(r"(-?\d+\.?\d*)\s*dB", ln, re.I).group(1))
                    except Exception:
                        pass
                if "sinr" in s and sinr is None:
                    try:
                        sinr = float(re.search(r"(-?\d+\.?\d*)\s*dB", ln, re.I).group(1))
                    except Exception:
                        pass
    ...
    # ---------- Wi-Fi / Cell buttons wiring already done ----------

...
import queue, datetime, xml.etree.ElementTree as ET, re, csv, io, os, math, hashlib, sqlite3, json, time, zipfile
...
class BluetoothApp:
    ...
    def __init__(self, root):
        ...
        self.obex_button.grid(row=1, column=7, padx=4, pady=5)
        self.pbap_button = tk.Button(self.device_frame, text="Contacts (PBAP/FTP)", command=self.open_contacts_browser)
        self.pbap_button.grid(row=1, column=8, padx=4, pady=5)
        self.map3d_button = tk.Button(self.device_frame, text="3D Map", command=self.open_map3d_popup)
        self.map3d_button.grid(row=1, column=9, padx=4, pady=5)
        self.open_map_button = tk.Button(self.device_frame, text="Open Map (Interactive)", command=self.open_interactive_map)
        self.open_map_button.grid(row=1, column=10, padx=4, pady=5)
        self.relabel_button = tk.Button(self.device_frame, text="Edit Details", command=self.relabel_selected_device)
        self.relabel_button.grid(row=1, column=11, padx=4, pady=5)
        self.clearlabel_button = tk.Button(self.device_frame, text="Clear Label", command=self.clear_label_selected_device)
        self.clearlabel_button.grid(row=1, column=12, padx=4, pady=5)
        self.scan_paired_button = tk.Button(self.device_frame, text="Scan Paired (Windows)", command=self.start_scan_paired_windows)
        self.scan_paired_button.grid(row=1, column=13, padx=4, pady=5)

        self.send_button = tk.Button(self.device_frame, text="Send Data", command=self.send_data_to_device)
        self.send_button.grid(row=2, column=0, padx=4, pady=5)
        self.receive_button = tk.Button(self.device_frame, text="Receive Data", command=self.receive_data_from_device)
        self.receive_button.grid(row=2, column=1, padx=4, pady=5)
        self.advertise_button = tk.Button(self.device_frame, text="Advertise 'Bullish'", command=self.start_advertising)
        self.advertise_button.grid(row=2, column=2, padx=4, pady=5)
        self.download_button = tk.Button(self.device_frame, text="Export Selected/List", command=self.download_info)
        self.download_button.grid(row=2, column=3, padx=4, pady=5)
        self.export_all_button = tk.Button(self.device_frame, text="Export ALL Radios (ZIP)", command=self.download_all_radio_data)
        self.export_all_button.grid(row=2, column=4, padx=4, pady=5)
        self.save_log_button = tk.Button(self.device_frame, text="Save Log", command=self.save_log_to_file)
        self.save_log_button.grid(row=2, column=5, padx=4, pady=5)
        self.save_map_html_button = tk.Button(self.device_frame, text="Save Map HTML", command=self.save_interactive_map)
        self.save_map_html_button.grid(row=2, column=6, padx=4, pady=5)

        # Tracking and Anchors
        self.tracking_enabled_var = tk.BooleanVar(value=True)
        tk.Checkbutton(self.device_frame, text="Track live", variable=self.tracking_enabled_var).grid(row=2, column=7, padx=4, pady=5, sticky="w")
        tk.Button(self.device_frame, text="Tracker View", command=self.open_tracker_view).grid(row=2, column=8, padx=4, pady=5)
        tk.Checkbutton(self.device_frame, text="Broadcast anchors", variable=self.anchor_broadcast_enabled,
                       command=self._on_anchor_toggle).grid(row=2, column=9, padx=6, pady=5, sticky="w")
        tk.Checkbutton(self.device_frame, text="Listen anchors", variable=self.anchor_listen_enabled,
                       command=self._on_anchor_toggle).grid(row=2, column=10, padx=6, pady=5, sticky="w")

        # Device category filters for the list view
        self.filter_frame = tk.Frame(self.device_frame)
        self.filter_frame.grid(row=3, column=0, columnspan=18, sticky="w", padx=5, pady=(0, 5))
        tk.Label(self.filter_frame, text="Show:").pack(side="left", padx=(0, 6))
        self.filter_vars = {}
        self._filter_order = [
            ("BLE", "Bluetooth LE"),
            ("Classic", "Classic BT"),
        ...
        for key, label in self._filter_order:
    ...
    def _disable_classic_io(self):
        for btn in (self.send_button, self.receive_button, self.advertise_button):
            try:
                btn.configure(state="disabled")
            except Exception:
                pass
        for btn in (self.obex_button, self.pbap_button):
            try:
                btn.configure(state="disabled")
            except Exception:
                pass
    ...
    def _init_db(self):
        try:
            ...
            with self.db_lock:
                ...
                cur.execute("""CREATE TABLE IF NOT EXISTS devices (addr TEXT PRIMARY KEY, label TEXT, image BLOB)""")
                cur.execute("PRAGMA table_info(devices)")
                cols = [row[1] for row in cur.fetchall()]
                if "image" not in cols:
                    cur.execute("ALTER TABLE devices ADD COLUMN image BLOB")
                self.db.commit()
        except Exception as e:
            self.log(f"DB init error: {e}")

    def db_get_device_details(self, addr: str):
        try:
            with self.db_lock:
                cur = self.db.cursor()
                cur.execute("SELECT label, image FROM devices WHERE addr=?", (addr,))
                row = cur.fetchone()
            if not row:
                return {"label": None, "image": None}
            label, image = row
    ...
    def db_get_label(self, addr: str):
        return self.db_get_device_details(addr)["label"]

    def db_get_image(self, addr: str):
        return self.db_get_device_details(addr)["image"]

    def db_set_device_details(self, addr: str, label: str, image_bytes):
        try:
            with self.db_lock:
                cur = self.db.cursor()
                cur.execute(
                    "INSERT INTO devices(addr,label,image) VALUES(?,?,?) "
                    "ON CONFLICT(addr) DO UPDATE SET label=excluded.label, image=excluded.image",
                    (addr, label, sqlite3.Binary(image_bytes) if image_bytes is not None else None),
                )
                self.db.commit()
        except Exception as e:
    ...
    def db_set_label(self, addr: str, label: str):
        existing_image = self.db_get_image(addr)
        self.db_set_device_details(addr, label, existing_image)
    ...
    def db_clear_label(self, addr: str):
        try:
            with self.db_lock:
                cur = self.db.cursor()
                cur.execute(
                    "INSERT INTO devices(addr,label) VALUES(?,NULL) "
                    "ON CONFLICT(addr) DO UPDATE SET label=NULL",
                    (addr,),
                )
                self.db.commit()
        except Exception as e:
            self.log(f"DB clear label error: {e}")
    ...
    def _apply_label_update(self, addr: str, new_label):
        for i, (t, d) in enumerate(self.devices):
            if t == "BLE" and d.address == addr:
                meta = self.ble_meta.setdefault(addr, {})
                fallback = meta.get("fallback_name") or getattr(d, "name", None) or "[Unnamed Device]"
                meta.setdefault("fallback_name", fallback)
                if new_label:
                    meta["name"] = new_label
                else:
                    meta["name"] = fallback
                self.update_device_list_entry(i, self.connection_results.get(addr, "Not Tested"), d, "BLE")
            elif t == "Classic" and d["addr"] == addr:
                fallback = d.get("fallback_name") or d.get("name") or "[Unnamed Device]"
                d.setdefault("fallback_name", fallback)
                if new_label:
                    d["name"] = new_label
                else:
                    d["name"] = fallback
                self.update_device_list_entry(i, self.connection_results.get(addr, "Not Tested"), d, "Classic")
            elif t == "WiFi" and d.get("bssid") == addr:
                meta = self.wifi_meta.setdefault(addr, {})
                fallback = meta.get("fallback_ssid") or d.get("ssid") or "[Hidden SSID]"
                meta.setdefault("fallback_ssid", fallback)
                if new_label:
                    meta["ssid"] = new_label
                else:
                    meta["ssid"] = fallback
                self.update_device_list_entry(i, self.connection_results.get(addr, "Seen"), d, "WiFi")
            elif t == "Cell" and d.get("cid") == addr:
                meta = self.cell_meta.setdefault(addr, {})
                fallback = meta.get("fallback_operator") or d.get("operator") or "Unknown operator"
                meta.setdefault("fallback_operator", fallback)
                if new_label:
                    meta["operator"] = new_label
                else:
                    meta["operator"] = fallback
    ...
    def relabel_selected_device(self):
        ...
        details = self.db_get_device_details(addr)
        existing_label = details.get("label") or (cur_name or "")
        selected_image = details.get("image")

        dialog = tk.Toplevel(self.root)
        dialog.title(f"Edit Details – {addr}")
        dialog.transient(self.root)
        dialog.grab_set()
        dialog.resizable(False, False)

        tk.Label(dialog, text="Custom Name:").grid(row=0, column=0, padx=10, pady=(10, 4), sticky="w")
        label_var = tk.StringVar(value=existing_label)
        entry = tk.Entry(dialog, textvariable=label_var, width=40)
        entry.grid(row=0, column=1, padx=10, pady=(10, 4), sticky="ew")

        tk.Label(dialog, text="Image:").grid(row=1, column=0, padx=10, pady=4, sticky="w")
        image_status = tk.StringVar()
        if selected_image:
            image_status.set(f"Stored image ({len(selected_image)} bytes)")
        else:
            image_status.set("No image selected")
        status_label = tk.Label(dialog, textvariable=image_status, anchor="w", width=40)
        status_label.grid(row=1, column=1, padx=10, pady=4, sticky="ew")
        ...
        def choose_image():
            ...
            try:
                with open(path, "rb") as fh:
                    data = fh.read()
                selected_image = data
                image_status.set(f"Selected {os.path.basename(path)} ({len(data)} bytes)")
            except Exception as e:
        ...
        btn_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=(0, 10), sticky="e")
        tk.Button(btn_frame, text="Choose…", command=choose_image).grid(row=0, column=0, padx=(0, 5))
        tk.Button(btn_frame, text="Clear Image", command=clear_image).grid(row=0, column=1)
        ...
        def on_save():
            ...
            if not new_label:
                ...
                return
            self.db_set_device_details(addr, new_label, selected_image)
            self._apply_label_update(addr, new_label)
            image_state = "set" if selected_image else "cleared"
            self.log(f"Updated details for {addr} (name='{new_label}', image {image_state})")
            dialog.destroy()

        def on_cancel():
            dialog.destroy()

        tk.Button(action_frame, text="Cancel", command=on_cancel).grid(row=0, column=0, padx=(0, 5))
        tk.Button(action_frame, text="Save", command=on_save).grid(row=0, column=1)
    ...
    def clear_label_selected_device(self):
        ...
        self._apply_label_update(addr, None)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    self.log(f"Error accepting connection: {exc}")
                    break
        except Exception as exc:
            self.log(f"Error during advertising: {exc}")
            self.queue.put(lambda: messagebox.showerror("Advertising Error", f"Error during advertising: {exc}"))
        finally:
            if self.server_sock:
    ...
    def prompt_user_for_connection(self, client_sock, address):
        if messagebox.askyesno("Connection Request", f"Accept connection from {address}?"):
    ...
    def handle_client(self, client_sock, address):
        ...
        try:
            ...
            self.log(f"Client {address} disconnected with error: {exc}")
        finally:
            try:
                client_sock.close()
            except Exception:
    ...
    def _get_active_filters(self):
        if not hasattr(self, "filter_vars"):
            return {"BLE", "Classic", "WiFi", "Cell"}
        return {key for key, var in self.filter_vars.items() if var.get()}
    ...
    def redraw_device_list(self, append_only=False):
        ...
        for device_type, dev in self.devices:
            if device_type not in active_filters:
                continue
            if device_type == "BLE":
    ...
    def save_interactive_map(self):
        if not HAVE_FOLIUM:
            messagebox.showerror("Map Unavailable", "Install 'folium' to enable saving the map.\n\npip install folium")
            return
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        dest = filedialog.asksaveasfilename(
            defaultextension=".html",
            initialfile=f"radio_map_{ts}.html",
            filetypes=[("HTML", "*.html")],
        )
        if not dest:
            return
        try:
            self._generate_interactive_map(out_path=dest)
            self.log(f"Saved interactive map -> {dest}")
            messagebox.showinfo("Saved", f"Saved to:\n{dest}")
        except Exception as exc:
            self.log(f"Save Map failed: {exc}")
    ...
    def save_log_to_file(self):
        ...
        dest = filedialog.asksaveasfilename(
            ...
            initialfile=f"radio_log_{ts}.txt",
            filetypes=[("Text", "*.txt")],
        )
        if not dest:
            return
        try:
            ...
            with open(dest, "w", encoding="utf-8") as f:
                f.write(content)
            self.log(f"Saved log -> {dest}")
            messagebox.showinfo("Saved", f"Saved to:\n{dest}")
        except Exception as exc:
            self.log(f"Save log failed: {exc}")
            messagebox.showerror("Save Failed", str(exc))
    ...
    def download_all_radio_data(self):
        """Export all known radio data into a single ZIP archive."""
        if not self.devices:
            messagebox.showwarning("Nothing to export", "No radios found yet. Try scanning first.")
            return

        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        dest = filedialog.asksaveasfilename(
            defaultextension=".zip",
            initialfile=f"radio_export_{ts}.zip",
        ...
        data = {
            ...
            "gps_origin": {"lat": self.gps_lat, "lon": self.gps_lon},
            "counts": {"ble": 0, "classic": 0, "wifi": 0, "cell": 0},
            "ble": [],
            "classic": [],
            "wifi": [],
            "cell": [],
        ...
        for typ, dev in self.devices:
            try:
                if typ == "BLE":
                    addr = dev.address
                    m = self.ble_meta.get(addr, {}) or {}
                    data["ble"].append({
                        "address": addr,
                        "name": m.get("name") or m.get("fallback_name") or getattr(dev, "name", None) or "[Unnamed Device]",
                        "status": self.connection_results.get(addr, "Not Tested"),
                        "rssi_dbm": m.get("rssi"),
                    ...
                    bssid = dev["bssid"]
                    m = self.wifi_meta.get(bssid, {}) or {}
                    data["wifi"].append({
                        "bssid": bssid,
                        "ssid": m.get("ssid") or dev.get("ssid"),
                        "status": self.connection_results.get(bssid, "Seen"),
                        "rssi_dbm": m.get("rssi"),
                        "distance_m": m.get("distance_m"),
                        "range_label": m.get("range_label"),
                        "quality_grade": m.get("qual"),
                    ...
                    cid = dev["cid"]
                    m = self.cell_meta.get(cid, {}) or {}
                    data["cell"].append({
                        "cid": cid,
                        "operator": m.get("operator"),
                        "tech": m.get("tech"),
                        "status": self.connection_results.get(cid, "Seen"),
        ...
        data["counts"] = {
            "ble": len(data["ble"]),
            "classic": len(data["classic"]),
            "wifi": len(data["wifi"]),
            "cell": len(data["cell"]),
        }

        def _csv_bytes(rows, prefer_cols=None):
            rows = rows or []
            all_keys = set()
            for r in rows:
            ...
            header = []
            if prefer_cols:
                header.extend([k for k in prefer_cols if k in all_keys])
        ...
        try:
            with zipfile.ZipFile(dest, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                zf.writestr("all_data.json", json.dumps(data, ensure_ascii=False, indent=2))
                zf.writestr(
                    "ble.csv",
                    _csv_bytes(
                        data["ble"],
                ...
                zf.writestr(
                    ...
                    _csv_bytes(
                        data["classic"],
                        prefer_cols=["address", "name", "status", "device_class", "type_str", "services", "device_id"],
                    ),
                )
                zf.writestr(
                    "wifi.csv",
                ...
                displayed = "\n".join(self._get_displayed_list_lines())
                zf.writestr("displayed_list.txt", displayed)

                try:
                    log_text = self.log_text.get("1.0", tk.END)
                except Exception:
                    log_text = ""
                zf.writestr("log.txt", log_text)

                try:
                    if self.explorer_text:
                        explorer_dump = self.explorer_text.get("1.0", tk.END)
                        if explorer_dump.strip():
                            zf.writestr("ble_services_last.txt", explorer_dump)
                except Exception:
                    pass

            self.log(f"Exported ALL radio data -> {dest}")
            messagebox.showinfo("Export Complete", f"Saved ZIP to:\n{dest}")
        except Exception as exc:
            self.log(f"Export ALL failed: {exc}")

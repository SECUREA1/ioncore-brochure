...
def launch_gui(auto_open_dialog=True):
    ...
    class App(tk.Tk):
        def __init__(self):
            super().__init__()
            self.title("MAC / BLE Inspector (GUI)")
            ...
            self.status = tk.StringVar(value="Ready"); ttk.Label(self, textvariable=self.status, anchor="w", padding=(8,4)).pack(side=tk.BOTTOM, fill=tk.X)
